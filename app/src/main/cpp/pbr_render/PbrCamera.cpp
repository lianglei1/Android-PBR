...
PbrCamera::PbrCamera(glm::vec3 position, glm::vec3 up, glm::vec3 target)
...
PbrCamera::PbrCamera(float posX, float posY, float posZ, float upX, float upY, float upZ, float targetX, float targetY, float targetZ)
	: mMovementSpeed(SPEED)
	, mMouseSensitivity(SENSITIVITY)
	, mZoom(ZOOM)
{
...
glm::mat4 PbrCamera::getViewMatrix()
{
	return glm::lookAt(mPosition, mTarget, mUp);
}

void PbrCamera::processMouseMovement(float xoffset, float yoffset)
{
	if (mDragging) {
		...
		focusVector = glm::vec3((rotation * glm::vec4(focusVector, 1.0f)));
...
void PbrCamera::processMousePress(bool leftPressed, bool rightPressed)
{
...
void PbrCamera::processMouseScroll(float yoffset)
{
	yoffset *= -SPEED;
...
void PbrCamera::updateCameraVectors()
{
