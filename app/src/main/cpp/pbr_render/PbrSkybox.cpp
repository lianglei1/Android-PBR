...
namespace {
	PbrVertex make_vertex(float px, float py, float pz, float tx, float ty, float tz, float nx, float ny, float nz, float u, float v) {
...
void PbrSkybox::setEnvironmentMap(std::shared_ptr<PbrCubeMap> environmentMap)
{
	mEnvironmentMap = environmentMap;
}

void PbrSkybox::draw(const std::shared_ptr<PbrShader>  shader)
{
